...
class WorkflowContractTests(unittest.TestCase):
    ...
    def test_copilot_lane_dispatches_async_tasks_without_local_agent_execution(self) -> None:
        for text in (WORKFLOW, ROOT_ACTION):
            ...
            self.assertIn("reachctl remediate . \\", text)
            self.assertIn("--agent copilot", text)
            self.assertIn("--output-dir .reachable/remediation-bundle", text)
            self.assertIn("reachctl copilot doctor", text)
            self.assertIn('--repo "${GITHUB_REPOSITORY}"', text)
            self.assertIn("--dispatch-kind agent_task", text)
    ...
    def test_setup_and_pr_actions_have_ci_fallbacks(self) -> None:
        ...
        self.assertIn('public_installer_url="https://sthenosec.com/download/install.sh"', setup)
        self.assertIn('fallback_installer_url="https://raw.githubusercontent.com/${{ inputs.dist-repo }}/main/install.sh"', setup)
        self.assertIn('Reachable public installer attempt ${attempt} failed; trying fallback repo installer.', setup)
    ...
    def test_root_action_is_the_marketplace_entrypoint(self) -> None:
        ...
        self.assertIn("uses: sthenos-security/reach-ci-github/actions/setup-reachable@v1", ROOT_ACTION)
        self.assertIn("uses: sthenos-security/reach-ci-github/actions/remediation-core@v1", ROOT_ACTION)
        self.assertIn("uses: sthenos-security/reach-ci-github/actions/open-remediation-pr@v1", ROOT_ACTION)
